...
// ===============================
// 📅 Función para fechas locales
// ===============================
function getLocalDate(dateStr) {
  const [year, month, day] = dateStr.split("-").map(Number);
  return new Date(year, month - 1, day, 0, 0, 0); // siempre a las 00:00 locales
}

// ===============================
// 📅 Configuración de fechas
// ===============================
const startDate = getLocalDate("2025-08-02");
...
function renderMilestones() {
  ...
  // Último pasado
  if (lastPast) {
    const date = getLocalDate(lastPast.date);
  ...
  if (nextFuture) {
    ...
    function updateCountdown() {
      const now = new Date();
      const target = getLocalDate(nextFuture.date);
